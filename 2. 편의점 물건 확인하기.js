...
function 재고확인(menu, num) {
  const checklist = list[menu];
  if (!checklist) return console.log(`"${menu}" (품목 없음)`);
  if (num !== checklist)
    return console.log(`"전산표와 일치하지 않습니다." (불일치)`);
  if (num == checklist) return console.log(`"전산표와 일치합니다." (일치)`);
}
